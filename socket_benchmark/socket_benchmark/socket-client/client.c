...
bool throughput_write(int epfd, int sockfd, char *buf);
...
int do_send(int sockfd, char *buf);
...
int main(int argc, char *argv[])
{
    ...
    printf("waiting 3 seconds, and then start sending data\n");
    //sleep(2);

    for (int i = 0; i < conn_num; i++)
    {
        do_send(socket_set[i], buf);
    }
...
static void handle_send(int epfd, int sockfd, char *buf)
{
    ...
    else
    {
        ...
        {
            throughput_write(epfd, sockfd, buf);
            printf("[+]: fd=%d closed\n", sockfd);
            close(sockfd);
        }
    }
